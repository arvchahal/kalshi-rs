...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 1. Get all active markets a lot of stuff not gonna lie not reccomended

    println!("1. Getting active markets...");
    let active_params = MarketsQuery {
        limit: Some(5), //default is 100 if left as None
        cursor: None,
        event_ticker: None,
        series_ticker: None,
        max_close_ts: None,
    ...
    // 2. Filter by series ticker somewhat better i guess
    ...
    // 4. Pagination example ie use cursor
    ...
    loop {
        let page_params = MarketsQuery {
            limit: Some(3), // Small page size for demo
            cursor: cursor.clone(),
            event_ticker: None,
            series_ticker: None,
            max_close_ts: None,
