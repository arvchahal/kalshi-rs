...
async fn test_get_all_structured_targets() {
    ...
    if !resp.structured_targets.is_empty() {
        ...
        println!("First target: ID={}, Name={}, Type={}", first.id, first.name, first.r#type);
    }
}
...
async fn test_get_all_structured_targets_with_limit() {
    ...
    // Note: The API may not respect the limit parameter for this endpoint
...
async fn test_structured_targets_endpoints_comprehensive() {
    ...
    // Test 1: Get all structured targets
    ...
    // Test 2: Get details for first target
    ...
    // Test 3: Verify all targets have ID and type (basic structure)
...
async fn test_structured_targets_pagination() {
    ...
    if let Some(cursor) = &page1.cursor {
        ...
        // Verify we got different targets (by checking IDs don't overlap)
        let page1_ids: Vec<&str> = page1.structured_targets.iter().map(|t| t.id.as_str()).collect();
        let page2_ids: Vec<&str> = page2.structured_targets.iter().map(|t| t.id.as_str()).collect();

        // Check that page2 has some targets
        assert!(!page2.structured_targets.is_empty(), "Page 2 should have targets");

        // Check that page1 and page2 don't have the same first target
        if !page1_ids.is_empty() && !page2_ids.is_empty() {
            assert_ne!(page1_ids[0], page2_ids[0], "Page 2 should have different targets than page 1");
        }
    } else {
