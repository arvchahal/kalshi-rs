...
async fn test_get_quotes_list() {
    ...
    let result = client
        .get_quotes(None, None, None, Some(10), None, None, creator_rfq_id, None)
        .await;
    assert!(result.is_ok(), "Failed to get quotes: {:?}", result.err());
    sleep(Duration::from_secs(2)).await;
    let _deleted = client.delete_rfq(&rfq_id).await.expect("Failed to delete RFQ");
    let resp = result.unwrap();
...
async fn test_get_communications_id() {
    ...
    let resp = result.unwrap();
    assert!(! resp.communications_id.is_empty(), "Expected non-empty communications ID");
}
#[tokio::test]
...
async fn test_create_and_delete_rfq_lifecycle() {
    ...
    let created = client.create_rfq(&rfq_body).await.expect("Failed to create RFQ");
    sleep(Duration::from_secs(2)).await;
    let _deleted = client.delete_rfq(&created.id).await.expect("Failed to delete RFQ");
}
#[tokio::test]
...
async fn test_create_quote_and_accept_flow() {
    ...
    let created = client.create_quote(quote_body).await.expect("Failed to create quote");
    sleep(Duration::from_secs(2)).await;
